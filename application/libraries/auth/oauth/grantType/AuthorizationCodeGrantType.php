...
class AuthorizationCodeGrantType implements GrantTypeInterface
{
    ...
    public function validateRequest(RequestInterface $request, ResponseInterface $response): bool
    {
        ...
        if (isset($authCode['redirect_uri']) && $authCode['redirect_uri']) {
            if (!$request->request('redirect_uri') || urldecode($request->request('redirect_uri')) !== rawurldecode($authCode['redirect_uri'])) {
//                $response->setError(400, 'redirect_uri_mismatch', "The redirect URI is missing or do not match", "#section-4.1.3");
                $response->setError(400, 'invalid_grant', "The redirect URI is missing or do not match", "#section-4.1.3");
