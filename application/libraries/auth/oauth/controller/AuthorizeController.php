<?php
namespace libraries\auth\oauth\controller;
...
class AuthorizeController implements AuthorizeControllerInterface
{
     ...
     * @param ScopeInterface|null $scopeUtil OPTIONAL Instance of OAuth2\ScopeInterface to validate the requested scope
     * @code
     *     $config = array(
     ...
     */
    public function handleAuthorizeRequest(RequestInterface $request, ResponseInterface $response, $is_authorized, $user_id = null): void
    {
        ...
        // add response parameters to query
        $response->addParameters($uri_params['query']);
    }
     ...
     * @param mixed             $user_id
     */
    protected function setNotAuthorizedResponse(RequestInterface $request, ResponseInterface $response, $redirect_uri, $user_id = null): void
    {
        $error = 'access_denied';
...
//            $response->setError(400, 'invalid_client', "No client id supplied");
            $response->setError(400, 'invalid_request', "No client id supplied");

            return false;
        }

        /*
         * add logic to validate redirect uri
         */
        // Make sure a valid redirect uri was supplied (we can not redirect because we were unable to verify the URI)
        if (!$supplied_redirect_uri = $request->query('redirect_uri', $request->request('redirect_uri'))) {
            // We don't have a good URI to use
            $response->setError(400, 'invalid_request', "No redirect uri supplied");
...
//                $response->setError(400, 'redirect_uri_mismatch', 'The redirect URI provided is missing or does not match', '#section-3.1.2');
                $response->setError(400, 'invalid_grant', 'The redirect URI provided is missing or does not match', '#section-3.1.2');
        ...
        // type and client_id are required
        if (!in_array($response_type, $this->getValidResponseTypes(), true)) {
            $response->setRedirect($this->config['redirect_status_code'], $redirect_uri, $state, 'unsupported_response_type', 'Invalid or missing response type', null);
...
//                $response->setError(400, 'redirect_uri_mismatch', 'The redirect URI is mandatory and was not supplied');
                $response->setError(400, 'invalid_grant', 'The redirect URI is mandatory and was not supplied');
...
//            if (false === $defaultScope) {
    ...
    protected function getValidResponseTypes(): array
    {
        return [
     ...
     * @see http://tools.ietf.org/html/rfc6749#section-3.1.2
     */
    protected function validateRedirectUri($inputUri, $registeredUriString): bool
    {
        if (!$inputUri || !$registeredUriString) {
            return false; // if either one is missing, assume INVALID
        }

        $registeredUris = explode($this->config['redirect_uri_delimiter'], $registeredUriString);
        foreach ($registeredUris as $registeredUri) {
            $registered_uris = preg_split('/\s+/', rawurldecode($registeredUri));
