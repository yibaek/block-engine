...
//            if (false === $defaultScope) {
    ...
    public function revokeToken(RequestInterface $request, ResponseInterface $response)
    {
        ...
        if (!in_array($token_type_hint, array(null, 'access_token', 'refresh_token'), true)) {
            $response->setError(400, 'unsupported_token_type', 'Token type hint must be either \'access_token\' or \'refresh_token\'');
        ...
        $revoked = $this->accessToken->revokeToken($token, $token_type_hint, $request->request('client_id'));
        if (!$revoked) {
            $response->setError(400, 'invalid_grant', 'The token provided is invalid');
            return null;
        }
