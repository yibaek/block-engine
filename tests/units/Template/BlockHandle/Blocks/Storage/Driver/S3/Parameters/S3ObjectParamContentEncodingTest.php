...
use Ntuple\Synctree\Template\BlockHandle\Blocks\Miscellaneous\ProtocolContentEncoding;
 ...
 * @since SRT-231
 */
class S3ObjectParamContentEncodingTest extends TestCase
{
    /**
     * @test
    ...
    public function block_returns_tuple_contains_param_key_and_value(string $mockValue)
    {
        ...
        $sut = new S3ObjectParamContentEncoding($storage, $extra, $value);
        ...
        $this->assertEquals(S3ObjectParamContentEncoding::PARAM_KEY, $result[0]);
    ...
    {
        foreach (ProtocolContentEncoding::OPTIONS as $value) {
            yield $value => [$value];
        }
    }
    ...
    /**
     * @test
     * @testdox 블럭은 `Content-Encoding`을 표현할 수 있는 `string`만을 값으로 받는다.
