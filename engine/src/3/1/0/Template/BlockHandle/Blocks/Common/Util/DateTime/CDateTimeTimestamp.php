...
class CDateTimeTimestamp implements IBlock
{
    use BlockHandleTrait;

    public const TYPE = 'common-util';
    public const ACTION = 'datetime-timestamp';
    ...
    /**
     * CDateTimeTimestamp constructor.
    ...
    {
        try {
            return ($this->getDateTime($blockStorage))->getUnixTimestamp();
        } catch (SynctreeException|SynctreeInnerException $ex) {
            throw $ex;
        } catch (Throwable $ex) {
            $this->storage->getLogger()->exception($ex, self::TYPE.':'.self::ACTION);
            throw (new RuntimeException('Util-DateTime-Timestamp'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function getDateTime(array &$blockStorage): DateTimeSupport
    {
        ...
        if (!$datetime instanceof DateTimeSupport) {
            throw (new InvalidArgumentException('Util-DateTime-Timestamp: Invalid datetime'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
