...
class CDecimalToFloat implements IBlock
{
    use BlockHandleTrait;

    public const TYPE = 'common-util';
    public const ACTION = 'decimal-to-float';
     ...
     */
    public function do(array &$blockStorage): float
    {
        try {
            return $this->getDecimal($blockStorage)->asFloat();
            ...
            throw (new RuntimeException('Util-Decimal-toFloat'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function getDecimal(array &$blockStorage): DecimalSupport
    {
        ...
        if (!$decimal instanceof DecimalSupport) {
            throw (new InvalidArgumentException('Util-Decimal-toFloat: Invalid decimal: Not a decimal type'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
