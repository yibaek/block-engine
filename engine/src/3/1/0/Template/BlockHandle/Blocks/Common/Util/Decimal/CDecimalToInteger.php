...
class CDecimalToInteger implements IBlock
{
    use BlockHandleTrait;

    public const TYPE = 'common-util';
    public const ACTION = 'decimal-to-integer';
    ...
    {
        try {
            return $this->getDecimal($blockStorage)->asInteger();
            ...
            throw (new RuntimeException('Util-Decimal-toInteger'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function getDecimal(array &$blockStorage): DecimalSupport
    {
        ...
        if (!$decimal instanceof DecimalSupport) {
            throw (new InvalidArgumentException('Util-Decimal-toInteger: Invalid decimal: Not a decimal type'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
