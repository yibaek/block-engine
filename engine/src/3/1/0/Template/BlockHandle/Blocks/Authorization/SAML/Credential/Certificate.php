...
class Certificate implements IBlock
{
    use BlockHandleTrait;

    public const TYPE = 'authorization';
    public const ACTION = 'saml-credential-certificate';
    ...
    public function do(array &$blockStorage): string
    {
        try {
            ...
            throw (new RuntimeException('Authorization-SAML-Credential-Certificate'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function getValue(array &$blockStorage): string
    {
        ...
        if (!is_string($value)) {
            throw (new InvalidArgumentException('Authorization-SAML-Credential-Certificate: Invalid value: Not a string type'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
