...
class CHashMapRemove implements IBlock
{
    ...
    public function do(array &$blockStorage): void
    {
        try {
            ...
            foreach($this->storage->getStackManager()->toArray() as $stack) {
                if (array_key_exists($arrayName, $stack->data)) {
                    if (!is_array($stack->data[$arrayName])) {
                        throw (new InvalidArgumentException('HashMap-Remove: Not a array type: '.$arrayName))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
                    }
            ...
            if (!array_key_exists($arrayName, $blockStorage)) {
                throw (new InvalidArgumentException('HashMap-Remove: Not found data: '.$arrayName))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
            }
            ...
            throw (new RuntimeException('HashMap-Remove'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function getArrayName(array &$blockStorage)
    {
        try {
            ...
            throw (new InvalidArgumentException('HashMap-Remove: Invalid target key: '.$ex->getMessage()))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function getItemKey(array &$blockStorage, $keyName)
    {
        try {
            ...
            throw (new InvalidArgumentException('HashMap-Remove: Invalid item key: '.$ex->getMessage()))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function removeValue(array &$data, $keys): void
    {
        if (count($keys) === 1) {
            ...
            if (!array_key_exists($key, $data)) {
                throw (new InvalidArgumentException('HashMap-Remove: Not found data: '.$key))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
            }
            ...
            if (!is_array($data[$key])) {
                throw (new InvalidArgumentException('HashMap-Remove: Not a array type: '.$key))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
            }
