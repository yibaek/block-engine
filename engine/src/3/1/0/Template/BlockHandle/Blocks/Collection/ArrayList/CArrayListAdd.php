...
class CArrayListAdd implements IBlock
{
    ...
    public function do(array &$blockStorage): void
    {
        try {
            ...
            foreach($this->storage->getStackManager()->toArray() as $stack) {
                if (array_key_exists($arrayName, $stack->data)) {
                    if (!is_array($stack->data[$arrayName])) {
                        throw (new InvalidArgumentException('ArrayList-Add: Not a array type: '.$arrayName))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
                    }
                    $stack->data[$arrayName] = $this->addValue($stack->data[$arrayName], $keys, $this->value->do($blockStorage));
                    return;
                }
            }

            if (!array_key_exists($arrayName, $blockStorage)) {
                throw (new InvalidArgumentException('ArrayList-Add: Not found data: '.$arrayName))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
            }
            ...
            $blockStorage[$arrayName] = $this->addValue($blockStorage[$arrayName], $keys, $this->value->do($blockStorage));
        } catch (SynctreeException|SynctreeInnerException $ex) {
            throw $ex;
        } catch (Throwable $ex) {
            $this->storage->getLogger()->exception($ex, self::TYPE.':'.self::ACTION);
            throw (new RuntimeException('ArrayList-Add'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function getArrayName(array &$blockStorage)
    {
        try {
            ...
            throw (new InvalidArgumentException('ArrayList-Add: Invalid target key: '.$ex->getMessage()))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function getItemKey(array &$blockStorage, $keyName)
    {
        try {
            ...
            throw (new InvalidArgumentException('ArrayList-Add: Invalid item key: '.$ex->getMessage()))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
     ...
     * @throws ISynctreeException
     */
    private function addValue(array $data, $keys, $putData)
    {
        if (is_array($keys) && !empty($keys)) {
            $key = array_shift($keys);

            if (!array_key_exists($key, $data)) {
                throw (new InvalidArgumentException('ArrayList-Add: Not found data: '.$key))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
            }

            if (!is_array($data[$key])) {
                throw (new InvalidArgumentException('ArrayList-Add: Not a array type: '.$key))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
            }
