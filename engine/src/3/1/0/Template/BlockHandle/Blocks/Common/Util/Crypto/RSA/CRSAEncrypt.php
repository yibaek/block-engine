...
class CRSAEncrypt implements IBlock
{
    use BlockHandleTrait;

    public const TYPE = 'common-util';
    public const ACTION = 'crypto-encrypt-rsa';
    ...
    {
        try {
            $isSuccess = openssl_public_encrypt($this->getValue($blockStorage), $encrypt, $this->getKey($blockStorage), $this->makeOption($blockStorage));
            if (false === $isSuccess) {
                $error = openssl_error_string() ?? '';
                throw (new CommonException('Util-Crypto-Encrypt-RSA: '.$error))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
            }
            return $encrypt;
            ...
            throw (new RuntimeException('Util-Crypto-Encrypt-RSA'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function getValue(array &$blockStorage): string
    {
        ...
        if (!is_string($value)) {
            throw (new InvalidArgumentException('Util-Crypto-Encrypt-RSA: Invalid value: Not a string type'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    ...
    private function getKey(array &$blockStorage): string
    {
        ...
        if (!is_string($key)) {
            throw (new InvalidArgumentException('Util-Crypto-Encrypt-RSA: Invalid key: Not a string type'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    ...
    private function makeOption(array &$blockStorage): int
    {
        ...
        foreach ($this->options as $option) {
            ...
            if (!is_int($value)) {
                throw (new InvalidArgumentException('Util-Crypto-Encrypt-RSA: Invalid option: Not a integer type'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
            }
            $options |= $value;
        }
