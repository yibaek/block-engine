...
class CDecimalMultiply implements IBlock
{
    use BlockHandleTrait;

    public const TYPE = 'common-util';
    public const ACTION = 'decimal-multiply';
    ...
    {
        try {
            return $this->getLeftValue($blockStorage)->mul($this->getRightValue($blockStorage), $this->getScale($blockStorage));
            ...
            throw (new RuntimeException('Util-Decimal-Multiply'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function getLeftValue(array &$blockStorage): DecimalSupport
    {
        ...
        if (!$leftValue instanceof DecimalSupport) {
            throw (new InvalidArgumentException('Util-Decimal-Multiply: Invalid left value: Not a decimal type'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    ...
    private function getRightValue(array &$blockStorage): DecimalSupport
    {
        ...
        if (!$rightValue instanceof DecimalSupport) {
            throw (new InvalidArgumentException('Util-Decimal-Multiply: Invalid right value: Not a decimal type'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    ...
    private function getScale(array &$blockStorage): ?int
    {
        ...
        if (!is_null($scale)) {
            if (!is_int($scale)) {
                throw (new InvalidArgumentException('Util-Decimal-Multiply: Invalid scale: Not a integer type'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
            }
        }
