...
class ReadLine implements IBlock
{
    use BlockHandleTrait;

    public const TYPE = 'file';
    public const ACTION = 'read-line';
    ...
    {
        try {
            return ($this->getFile($blockStorage))->readLine();
            ...
            throw (new RuntimeException('File-Read-Line'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function getFile(array &$blockStorage): FileSupport
    {
        ...
        if (!$file instanceof FileSupport) {
            throw (new InvalidArgumentException('File-Read-Line: Invalid file: Not a File type'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
