...
class RedisDel implements IBlock
{
    use BlockHandleTrait;

    public const TYPE = 'storage';
    public const ACTION = 'driver-redis-del';
    ...
    {
        try {
            return RedisUtil::del($this->getConnector($blockStorage), $this->getKey($blockStorage), $this->getDb($blockStorage));
            ...
            throw (new RuntimeException('Storage-Redis-Del'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function getConnector(array &$blockStorage): IRedisMgr
    {
        ...
        if (!$connector instanceof IRedisMgr) {
            throw (new InvalidArgumentException('Storage-Redis-Del: Invalid connector: Not a redis connector'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    ...
    private function getKey(array &$blockStorage): string
    {
        ...
        if (!is_string($key)) {
            throw (new InvalidArgumentException('Storage-Redis-Del: Invalid key: Not a string type'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    ...
    private function getDb(array &$blockStorage): int
    {
        ...
        if (!is_int($db)) {
            throw (new InvalidArgumentException('Storage-Redis-Del: Invalid db: Not a integer type'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
