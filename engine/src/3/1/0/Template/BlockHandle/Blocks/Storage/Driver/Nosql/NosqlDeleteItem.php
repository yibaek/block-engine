...
class NosqlDeleteItem implements IBlock
{
    use BlockHandleTrait;

    public const TYPE = 'storage';
    public const ACTION = 'driver-nosql-deleteitem';
    ...
    {
        try {
            return (new DynamoDbHandlerForCommon($this->storage, $this->getConnector($blockStorage)))->deleteItem($this->getKey($blockStorage));
            ...
            throw (new RuntimeException('Storage-Nosql-Delete'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function getConnector(array &$blockStorage): DynamoDbMgr
    {
        ...
        if (!$connector instanceof DynamoDbMgr) {
            throw (new InvalidArgumentException('Storage-Nosql-Delete: Invalid connector: Invalid connector type'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    ...
    private function getKey(array &$blockStorage): string
    {
        ...
        if (!is_string($key)) {
            throw (new InvalidArgumentException('Storage-Nosql-Delete: Invalid key: Not a string type'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
