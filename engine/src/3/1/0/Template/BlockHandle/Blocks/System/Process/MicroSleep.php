 ...
 * @since SRT-128
 */
class MicroSleep implements IBlock
{
    use BlockHandleTrait;

    public const TYPE = 'system';
    public const ACTION = 'process-micro-sleep';
     ...
     * @param IBlock|null $delay block returns positive integer
     */
    ...
    {
        try {
            $delay = $this->getDelay($blockStorage);
            usleep($delay);
        } catch (SynctreeException|SynctreeInnerException $ex) {
            throw $ex;
        } catch (Throwable $ex) {
            $this->storage->getLogger()->exception($ex, self::TYPE.':'.self::ACTION);
            throw (new RuntimeException('System-Process-MicroSleep', 0, $ex))
    ...
    public function getDelay(array &$blockStorage): int
    {
        ...
        if (!is_int($delay) || 0 > $delay) {
            throw (new InvalidArgumentException('System-Process-MicroSleep: delay must be a positive integer.'))
                ->setExceptionKey(self::TYPE, self::ACTION)
                ->setExtraData($this->extra->getData());
        }
