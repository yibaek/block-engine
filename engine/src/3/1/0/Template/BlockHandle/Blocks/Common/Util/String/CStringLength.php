...
class CStringLength implements IBlock
{
    use BlockHandleTrait;

    public const TYPE = 'common-util';
    public const ACTION = 'string-length';
    ...
    {
        try {
            return mb_strlen($this->getTarget($blockStorage), $this->getEncoding($blockStorage));
        } catch (SynctreeException|SynctreeInnerException $ex) {
            throw $ex;
        } catch (Throwable $ex) {
            $this->storage->getLogger()->exception($ex, self::TYPE.':'.self::ACTION);
            throw (new RuntimeException('Util-String-Length'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function getTarget(array &$blockStorage): string
    {
        try {
            ...
            throw (new InvalidArgumentException('Util-String-Length: Invalid target: '.$ex->getMessage()))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function getEncoding(array &$blockStorage): string
    {
        ...
        if (!is_string($encoding)) {
            throw (new InvalidArgumentException('Util-String-Length: Invalid encoding: Not a string type'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
