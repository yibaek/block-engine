...
class CHashMapGet implements IBlock
{
        ...
        try {
            ...
            foreach($this->storage->getStackManager()->toArray() as $stack) {
                if (array_key_exists($arrayName, $stack->data)) {
                    ...
                    if (!is_array($data)) {
                        throw (new InvalidArgumentException('HashMap-Get: Not a array type: '.$arrayName))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
                    }
                    break;
                }
            }
            ...
            if ($data === null) {
                if (!array_key_exists($arrayName, $blockStorage)) {
                    throw (new InvalidArgumentException('HashMap-Get: Not found data: '.$arrayName))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
                }
            ...
            foreach ($this->keysName as $keyName) {
                if (!is_array($data)) {
                    throw (new InvalidArgumentException('HashMap-Get: Not a array type: '.$previousKeyName))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
                }
            ...
            throw (new RuntimeException('HashMap-Get'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function getArrayName(array &$blockStorage)
    {
        try {
            ...
            throw (new InvalidArgumentException('HashMap-Get: Invalid target key: '.$ex->getMessage()))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function getItemKey(array &$blockStorage, $keyName)
    {
        try {
            ...
            throw (new InvalidArgumentException('HashMap-Get: Invalid item key: '.$ex->getMessage()))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function getValue(array $data, $key)
    {
        if (!array_key_exists($key, $data)) {
            throw (new InvalidArgumentException('HashMap-Get: Not found data: '.$key))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
