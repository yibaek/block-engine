...
class CStringLtrim implements IBlock
{
    use BlockHandleTrait;

    public const TYPE = 'common-util';
    public const ACTION = 'string-ltrim';
    ...
    {
        try {
            return ltrim($this->getTarget($blockStorage), $this->getMask($blockStorage));
        } catch (SynctreeException|SynctreeInnerException $ex) {
            throw $ex;
        } catch (Throwable $ex) {
            $this->storage->getLogger()->exception($ex, self::TYPE.':'.self::ACTION);
            throw (new RuntimeException('Util-String-LTrim'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function getTarget(array &$blockStorage): string
    {
        try {
            ...
            throw (new InvalidArgumentException('Util-String-LTrim: Invalid target: '.$ex->getMessage()))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function getMask(array &$blockStorage): string
    {
        ...
        if (!is_null($mask)) {
            if (!is_string($mask)) {
                throw (new InvalidArgumentException('Util-String-LTrim: Invalid mask: Not a string type'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
            }
        }
