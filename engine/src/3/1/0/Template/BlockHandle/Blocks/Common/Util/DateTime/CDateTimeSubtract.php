...
class CDateTimeSubtract implements IBlock
{
    use BlockHandleTrait;

    public const TYPE = 'common-util';
    public const ACTION = 'datetime-subtract';
    ...
    {
        try {
            return ($this->getDateTime($blockStorage))->sub($this->getIntervals($blockStorage), true);
        } catch (SynctreeException|SynctreeInnerException $ex) {
            throw $ex;
        } catch (Throwable $ex) {
            $this->storage->getLogger()->exception($ex, self::TYPE.':'.self::ACTION);
            throw (new RuntimeException('Util-DateTime-Subtract'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function getDateTime(array &$blockStorage): DateTimeSupport
    {
        ...
        if (!$datetime instanceof DateTimeSupport) {
            throw (new InvalidArgumentException('Util-DateTime-Subtract: Invalid datetime'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    ...
    private function getIntervals(array &$blockStorage): array
    {
        ...
        foreach ($this->intervals as $interval) {
            ...
            if (!is_array($data) || empty($data)) {
                throw (new InvalidArgumentException('Util-DateTime-Subtract: Invalid intervals'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
            }
