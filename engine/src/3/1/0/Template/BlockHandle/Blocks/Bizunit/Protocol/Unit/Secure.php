...
class Secure implements IBlock
{
    use BlockHandleTrait;

    public const TYPE = 'protocol-unit';
    public const ACTION = 'secure';
     ...
     * @param IBlock|null $verificationCode
     */
    public function __construct(PlanStorage $storage, ExtraManager $extra = null, IBlock $method = null, IBlock $endPoint = null, IBlock $header = null, IBlock $body = null, BlockAggregator $options = null, IBlock $verificationCode = null)
    {
    ...
    public function setData(array $data): IBlock
    {
        ...
        $this->verificationCode = $this->setBlock($this->storage, $data['template']['verification-code']);
    ...
    public function getTemplate(): array
    {
        return [
            ...
            'template' => [
                ...
                'verification-code' => $this->verificationCode->getTemplate()
            ]
        ];
    }
    ...
    public function do(array &$blockStorage): array
    {
        try {
            ...
            $executor = (new HttpExecutor($this->storage->getLogger(), (new HttpHandler($this->storage->getLogger()))->enableLogging()->getHandlerStack()))
                ...
                ->setBodys($reqBody)
                ->setSecureVerificationCode($this->getVerificationCode($blockStorage))
                ->isConvertJson(true);
            ...
            throw (new TransferException('SecureProtocol: '.$context['error'] ?? $ex->getMessage()))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
            ...
            throw (new RuntimeException('SecureProtocol'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function getMethod(array &$blockStorage): string
    {
        ...
        if (!is_string($method)) {
            throw (new InvalidArgumentException('SecureProtocol: Invalid method'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    ...
    private function getEndpoint(array &$blockStorage): string
    {
        ...
        if (!is_string($endpoint)) {
            throw (new InvalidArgumentException('SecureProtocol: Invalid endpoint'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    ...
    private function getHeader(array &$blockStorage): array
    {
        ...
        if (!is_array($header)) {
            throw (new InvalidArgumentException('SecureProtocol: Invalid header'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
     ...
     * @throws ISynctreeException
     */
    private function getVerificationCode(array &$blockStorage): string
    {
        $verificationCode = $this->verificationCode->do($blockStorage);
        if (!is_string($verificationCode)) {
            throw (new InvalidArgumentException('SecureProtocol: Invalid verification code'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
