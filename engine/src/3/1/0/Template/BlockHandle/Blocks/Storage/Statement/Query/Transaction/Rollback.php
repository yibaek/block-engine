...
class Rollback implements IBlock
{
    use BlockHandleTrait;

    public const TYPE = 'storage';
    public const ACTION = 'statement-query-transaction-rollback';
    ...
    public function do(array &$blockStorage)
    {
        try {
            ...
            $this->getConnector($blockStorage)->rollback();
            ...
            throw (new RuntimeException('Storage-Query-Transaction-Rollback'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function getConnector(array &$blockStorage): IRDbMgr
    {
        ...
        if (!$connector instanceof IRDbMgr) {
            throw (new InvalidArgumentException('Storage-Query-Transaction-Rollback: Invalid connector'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
