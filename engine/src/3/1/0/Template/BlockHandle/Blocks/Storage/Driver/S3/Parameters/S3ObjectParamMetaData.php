...
/**
 * S3 object metadata 설정
 *
 * @since SRT-201 S3 Option block 구현
 */
class S3ObjectParamMetaData implements IBlock
{
    use BlockHandleTrait;

    public const TYPE = 'storage';
    public const ACTION = 'driver-s3-object-param-metadata';

    /** @var string S3 parameter key */
    public const PARAM_KEY = 'Metadata';
    ...
    private function getValue(array &$blockStorage): array
    {
        ...
        if (!is_array($result)) {
            throw (new InvalidArgumentException(self::LABEL.': Invalid metadata; Not a array type'))
                ->setExceptionKey(self::TYPE, self::ACTION)
                ->setExtraData($this->extra->getData());
        } else if ([] === $result) {
            throw (new InvalidArgumentException(self::LABEL.': Invalid metadata; No data'))
                ->setExceptionKey(self::TYPE, self::ACTION)
                ->setExtraData($this->extra->getData());
        }
