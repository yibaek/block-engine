...
class CreateFromSecret implements IBlock
{
    use BlockHandleTrait;

    public const TYPE = 'authorization';
    public const ACTION = 'jwt-jwk-factory-secret';
    ...
    {
        try {
            return (new CreateJWK)->createFromSecret($this->getKey($blockStorage), $this->getAddValues($blockStorage));
        } catch (SynctreeException|SynctreeInnerException $ex) {
            throw $ex;
        } catch (Throwable $ex) {
            $this->storage->getLogger()->exception($ex, self::TYPE.':'.self::ACTION);
            throw (new RuntimeException('Authorization-JWK-Create-Secret'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function getKey(array &$blockStorage): string
    {
        ...
        if (!is_string($key)) {
            throw (new InvalidArgumentException('Authorization-JWK-Create-Secret: Invalid key: Not a string type'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    ...
    private function getAddValues(array &$blockStorage): ?array
    {
        ...
        if (!is_null($addValues)) {
            if (!is_array($addValues)) {
                throw (new InvalidArgumentException('Authorization-JWK-Create-Secret: Invalid add values: Not a array type'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
            }
        }
