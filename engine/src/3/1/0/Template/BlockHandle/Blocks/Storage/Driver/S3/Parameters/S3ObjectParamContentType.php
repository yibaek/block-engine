...
/**
 * S3 Object Mime-type 지정
 *
 * @since SRT-201 S3 Option block 구현
 */
class S3ObjectParamContentType implements IBlock
{
    use BlockHandleTrait;

    public const TYPE = 'storage';
    public const ACTION = 'driver-s3-object-param-content-type';

    /** @var string S3 parameter key */
    public const PARAM_KEY = 'ContentType';
     ...
     * @return string
     * @throws ISynctreeException
     * @see https://developer.mozilla.org/en/docs/Web/HTTP/Basics_of_HTTP/MIME_types/Common_types
     */
    private function getValue(array &$blockStorage): string
    {
        ...
        if (!is_string($result)) {
            throw (new InvalidArgumentException(self::LABEL.': Invalid mime-type; Not a string type'))
                ->setExceptionKey(self::TYPE, self::ACTION)
                ->setExtraData($this->extra->getData());
        }
