...
class CDateTimeDiffFormat implements IBlock
{
    use BlockHandleTrait;

    public const TYPE = 'common-util';
    public const ACTION = 'datetime-diff-format';
    ...
    /**
     * CDateTimeDiffFormat constructor.
    ...
    {
        try {
            return ($this->getDateTime($blockStorage))->diffFormat($this->getFormat($blockStorage));
        } catch (SynctreeException|SynctreeInnerException $ex) {
            throw $ex;
        } catch (Throwable $ex) {
            $this->storage->getLogger()->exception($ex, self::TYPE.':'.self::ACTION);
            throw (new RuntimeException('Util-DateTime-Diff-Format'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function getDateTime(array &$blockStorage): DateTimeSupport
    {
        ...
        if (!$datetime instanceof DateTimeSupport) {
            throw (new InvalidArgumentException('Util-DateTime-Diff-Format: Invalid datetime'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    ...
    private function getFormat(array &$blockStorage): string
    {
        ...
        if (!is_string($format)) {
            throw (new InvalidArgumentException('Util-DateTime-Diff-Format: Invalid format: Not a string type'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
