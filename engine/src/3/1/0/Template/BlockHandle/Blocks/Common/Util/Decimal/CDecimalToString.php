...
class CDecimalToString implements IBlock
{
    use BlockHandleTrait;

    public const TYPE = 'common-util';
    public const ACTION = 'decimal-to-string';
    ...
    {
        try {
            return (string) $this->getDecimal($blockStorage);
            ...
            throw (new RuntimeException('Util-Decimal-toString'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function getDecimal(array &$blockStorage): DecimalSupport
    {
        ...
        if (!$decimal instanceof DecimalSupport) {
            throw (new InvalidArgumentException('Util-Decimal-toString: Invalid decimal: Not a decimal type'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
