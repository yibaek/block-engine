...
class OracleExecuteProcedure implements IBlock
{
    use BlockHandleTrait;

    public const TYPE = 'storage';
    public const ACTION = 'driver-oracle-execute-procedure';
    ...
    /**
     * OracleExecuteProcedure constructor.
    ...
    public function do(array &$blockStorage)
    {
        try {
            ...
            // execute procedure
            return (new OracleHandler($this->getConnector($blockStorage)))->executeProcedure($procedure, $param);
            ...
            throw (new RuntimeException('Storage-Oracle-Execute-Procedure'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function getConnector(array &$blockStorage): OracleMgr
    {
        ...
        if (!$connector instanceof OracleMgr) {
            throw (new InvalidArgumentException('Storage-Oracle-Execute-Procedure: Invalid connector: Not a oracle connector'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    ...
    private function getProcedure(array &$blockStorage): array
    {
        ...
        if (!is_array($procedure)) {
            throw (new InvalidArgumentException('Storage-Oracle-Execute-Procedure: Invalid procedure: Not a array type'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
