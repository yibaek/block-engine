...
class CDecimalCeil implements IBlock
{
    use BlockHandleTrait;

    public const TYPE = 'common-util';
    public const ACTION = 'decimal-ceil';
    ...
    {
        try {
            return $this->getDecimal($blockStorage)->ceil($this->getScale($blockStorage));
            ...
            throw (new RuntimeException('Util-Decimal-Ceil'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function getDecimal(array &$blockStorage): DecimalSupport
    {
        ...
        if (!$decimal instanceof DecimalSupport) {
            throw (new InvalidArgumentException('Util-Decimal-Ceil: Invalid decimal: Not a decimal type'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    ...
    private function getScale(array &$blockStorage): ?int
    {
        ...
        if (!is_null($scale)) {
            if (!is_int($scale)) {
                throw (new InvalidArgumentException('Util-Decimal-Ceil: Invalid scale: Not a integer type'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
            }
        }
