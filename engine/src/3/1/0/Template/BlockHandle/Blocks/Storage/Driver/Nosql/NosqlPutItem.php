...
class NosqlPutItem implements IBlock
{
    use BlockHandleTrait;

    public const TYPE = 'storage';
    public const ACTION = 'driver-nosql-putitem';
    ...
    {
        try {
            return (new DynamoDbHandlerForCommon($this->storage, $this->getConnector($blockStorage)))->putItem($this->getKey($blockStorage), $this->getItem($blockStorage));
            ...
            throw (new RuntimeException('Storage-Nosql-Put'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    }
    ...
    private function getConnector(array &$blockStorage): DynamoDbMgr
    {
        ...
        if (!$connector instanceof DynamoDbMgr) {
            throw (new InvalidArgumentException('Storage-Nosql-Put: Invalid connector: Invalid connector type'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
    ...
    private function getKey(array &$blockStorage): string
    {
        ...
        if (!is_string($key)) {
            throw (new InvalidArgumentException('Storage-Nosql-Put: Invalid key: Not a string type'))->setExceptionKey(self::TYPE, self::ACTION)->setExtraData($this->extra->getData());
        }
