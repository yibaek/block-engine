...
class Parameter
{
    ...
    private function checkSpecialCharacter(string $name = null): string
    {
        ...
        if (preg_match('/[\'^£$%&*()}{@#~?><>,|=+¬-]/', $name)) {
            throw new MysqlStorageException('Invalid ouput parameter name: Special characters not supported: '.$name);
        }
    ...
    private function initType($value): string
    {
        ...
        switch ($type) {
            ...
            default:
                throw new MysqlStorageException('Invalid parameter type: '.$type);
        }
    }
}